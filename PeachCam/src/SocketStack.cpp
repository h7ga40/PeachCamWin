	...
	int optname, void *optval, unsigned *optlen)
{
	...
	case NSAPI_KEEPALIVE:
		if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
			if (::getsockopt(socket->id, SOL_SOCKET, SO_KEEPALIVE, (char *)optval, (int *)optlen)) {
				return NSAPI_ERROR_OK;
			}
		}
		break;
	case NSAPI_REUSEADDR:
		if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
			if (::getsockopt(socket->id, SOL_SOCKET, SO_REUSEADDR, (char *)optval, (int *)optlen)) {
				return NSAPI_ERROR_OK;
			}
		}
		break;
	case NSAPI_KEEPIDLE:
		if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
			if (::getsockopt(socket->id, IPPROTO_TCP, TCP_KEEPIDLE, (char *)optval, (int *)optlen)) {
				return NSAPI_ERROR_OK;
			}
		}
		break;
	case NSAPI_KEEPINTVL:
		if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
			if (::getsockopt(socket->id, IPPROTO_TCP, TCP_KEEPINTVL, (char *)optval, (int *)optlen)) {
				return NSAPI_ERROR_OK;
			}
		}
		break;
	...
	case NSAPI_NODELAY:
		if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
			if (::getsockopt(socket->id, IPPROTO_TCP, TCP_NODELAY, (char *)optval, (int *)optlen)) {
				return NSAPI_ERROR_OK;
			}
		}
		break;
	case NSAPI_ERROR:
		if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
			if (::getsockopt(socket->id, SOL_SOCKET, SO_ERROR, (char *)optval, (int *)optlen)) {
				return NSAPI_ERROR_OK;
			}
		}
		break;
	...
	case NSAPI_SO_RCVTIMEO:
		if (level == NSAPI_SOCKET && socket->proto == NSAPI_TCP) {
			if (::getsockopt(socket->id, SOL_SOCKET, SO_RCVTIMEO, (char *)optval, (int *)optlen)) {
				return NSAPI_ERROR_OK;
			}
		}
		break;
