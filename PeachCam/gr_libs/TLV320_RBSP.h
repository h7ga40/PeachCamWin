...
class R_BSP_Aio {
public:
	virtual int32_t write(void * const p_data, uint32_t data_size, const rbsp_data_conf_t * const p_data_conf = NULL) = 0;
	virtual int32_t read(void * const p_data, uint32_t data_size, const rbsp_data_conf_t * const p_data_conf = NULL) = 0;
};

class R_BSP_Ssif : public R_BSP_Aio {
public:
	R_BSP_Ssif(PinName sck, PinName ws, PinName tx, PinName rx, PinName audio_clk = NC);

	void init(ssif_channel_cfg_t *cfg, int32_t max_write_num, int32_t max_read_num);
	void ConfigChannel(ssif_channel_cfg_t *cfg);

	virtual int32_t write(void * const p_data, uint32_t data_size, const rbsp_data_conf_t * const p_data_conf = NULL);
	virtual int32_t read(void * const p_data, uint32_t data_size, const rbsp_data_conf_t * const p_data_conf = NULL);
