...
int equeue_create_inplace(equeue_t *q, size_t size, void *buffer) {
    ...
    q->chunks = 0;
    q->slab.size = size;
    q->slab.data = (unsigned char *)buffer;
...
int equeue_call(equeue_t *q, void (*cb)(void*), void *data) {
    struct ecallback *e = (struct ecallback *)equeue_alloc(q, sizeof(struct ecallback));
    if (!e) {
        return 0;
    }
...
void equeue_chain(equeue_t *q, equeue_t *target) {
    ...
    struct equeue_chain_context *c = (struct equeue_chain_context *)equeue_alloc(q,
