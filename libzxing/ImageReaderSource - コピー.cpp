...
Ref<LuminanceSource> ImageReaderSource::create(char* buf, int buf_size, int width, int height) {
    ...
    for (int x = 0; x < width; x++) {
        for (int y = 0; y < height; y++) {
            ...
            char t2 = buf[j++];
            char r = (t1 & 0xF8) | (t1 >> 5);
            char g = ((t1 & 0x07) << 5) | ((t2 & 0xE0) >> 3) | ((t1 & 0x07) >> 1);
            char b = ((t2 & 0x1F) << 3) | ((t2 & 0x1F) >> 2);
            srcimg_adr[i++] = b;
            srcimg_adr[i++] = g;
            srcimg_adr[i++] = r;
            srcimg_adr[i++] = 255;
...
#if INPUTIMG_RGB888 || INPUTIMG_RGB565    //1:RGB888 0:YCBCR422
